...
std::pair<int, int> Square::getCoordinates()
{
    ...
    coordinates.first = this->x_;
    coordinates.second = this->y_;
...
int Square::countAllMines()
{
    int mines = 0;

    for (int y = 0; y < board_->getSize(); ++y) {
        for (int x = 0; x < board_->getSize(); ++x) {
            Square currentSquare = board_->getSquare(x, y);
            if (currentSquare.hasMine() && not hasFlag()) {
                mines++;
            }
        }
    }
